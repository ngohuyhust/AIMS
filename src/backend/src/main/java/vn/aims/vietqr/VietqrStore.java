...
public class VietqrStore {
    record Row(int id,int paymentId,int orderId,BigDecimal amount,String content,String qrCode,String qrLink,String transactionId,String reference,Instant expires,String status,Instant created) {}
    ...
    Row byPayment(int id) {var rows=jdbc.query("SELECT * FROM vietqr_transactions WHERE transaction_id=?",mapper,id);return rows.isEmpty()?null:rows.getFirst();}
    List<Row> byReference(String ref) {return jdbc.query("SELECT * FROM vietqr_transactions WHERE transaction_ref_id=?",mapper,ref);}
    List<Row> byDetails(int order,String content,BigDecimal amount) {return jdbc.query("SELECT * FROM vietqr_transactions WHERE order_id=? AND content=? AND amount=?",mapper,order,content,amount);}
    List<Row> pending(int order) {return jdbc.query("SELECT * FROM vietqr_transactions WHERE order_id=? AND status='PENDING'",mapper,order);}
    void placeholder(int payment,int order,BigDecimal amount,String content,Instant expires) {
    ...
    void generated(int id,JsonNode qr) {
    ...
    void expire(Row row) {jdbc.update("UPDATE vietqr_transactions SET status='EXPIRED',updated_at=now() WHERE vietqr_transaction_id=? AND status='PENDING'",row.id());}
    void receipt(String bank,String bankId,int payment) {
    ...
    void paid(Row row,JsonNode callback) {
