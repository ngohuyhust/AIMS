package vn.aims.paypal.repository;
...
public class PaypalStore {
    public record OrderRow(int id,String status,BigDecimal total,String token) {}
    public record Binding(int transactionId,String method,BigDecimal amount,String paymentStatus,String gatewayId,String captureId,String status) {}
    public record Operation(UUID id,Instant createdAt,JsonNode response) {}
    private final JdbcTemplate jdbc;
    private final ObjectMapper json;
    public PaypalStore(JdbcTemplate jdbc,ObjectMapper json) { this.jdbc=jdbc;this.json=json; }
    public OrderRow order(int id) {
    ...
    public void placeholder(int id) { jdbc.update("INSERT INTO paypal_transactions(transaction_id,status) VALUES (?,'CREATING') ON CONFLICT(transaction_id) DO NOTHING",id); }
    public Binding binding(int orderId) {
        var rows=jdbc.query("""
    ...
    public void operation(int transactionId,String operation) {
    ...
    public Operation lockOperation(int transactionId,String operation) {
    ...
    public void response(UUID id,JsonNode response) { jdbc.update("UPDATE paypal_operations SET response=?::jsonb WHERE operation_id=?",response.toString(),id); }
    public void created(int id,String gatewayId,String status) { jdbc.update("UPDATE paypal_transactions SET paypal_order_id=?,status=? WHERE transaction_id=?",gatewayId,status,id); }
    public void captured(int id,String captureId) { jdbc.update("UPDATE paypal_transactions SET paypal_capture_id=?,status='COMPLETED' WHERE transaction_id=?",captureId,id); }
    public void refunded(int id) { jdbc.update("UPDATE paypal_transactions SET status='REFUNDED' WHERE transaction_id=?",id); }
}
