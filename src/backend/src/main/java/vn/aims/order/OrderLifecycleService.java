...
public class OrderLifecycleService {
    ...
    public Map<String,Object> approve(int id,String authorization) {
        manager(authorization);return transaction(()->{
            ...
            order.status="APPROVED";order.updatedAt=java.time.Instant.now();var result=OrderResponse.from(orders.save(order),false);
    ...
    public Map<String,Object> close(int id,String action,String token,String authorization,boolean customer) {
        ...
        boolean refund=transaction(()->{
            ...
            if(customer && (order.status.equals("PENDING_PROCESSING") || money.stream().anyMatch(p->!p.status().equals("PENDING")))) throw new OrderError(403,"Only a product manager can cancel a paid order");
            ...
            if(order.status.equals("PENDING_PROCESSING") && payment==null) throw new OrderError(409,"Paid order has no payment to reconcile");
        ...
        return transaction(()->{
            ...
            store.restore(id);order.status=payment!=null && payment.status()!=PaymentStatus.REFUNDED?"REFUND_PENDING":action.equals("CANCEL")?"CANCELLED":"REJECTED";
            order.updatedAt=java.time.Instant.now();store.complete(id);var result=OrderResponse.from(orders.save(order),customer);
    ...
    public Map<String,Object> confirmVietqrRefund(int id,String authorization) {
        manager(authorization);return transaction(()->{
            ...
            if(order.status.equals("REFUNDED") && payment.status().equals("REFUNDED")) return OrderResponse.from(order,false);
            if(!order.status.equals("REFUND_PENDING")) throw new OrderError(400,"Order #"+id+" is in status "+order.status+", not REFUND_PENDING");
            payments.markRefunded(new PaymentConfirmation(payment.id(),id,"VIETQR",payment.amount()));
            order.status="REFUNDED";order.updatedAt=java.time.Instant.now();var result=OrderResponse.from(orders.save(order),false);
