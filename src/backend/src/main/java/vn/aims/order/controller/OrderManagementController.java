package vn.aims.order.controller;

import org.springframework.http.HttpStatus;
import org.springframework.security.access.prepost.PreAuthorize;
import org.springframework.web.bind.annotation.*;
import vn.aims.order.dto.*;
import vn.aims.order.event.*;
import vn.aims.order.exception.*;
import vn.aims.order.service.*;
...
public class OrderManagementController {
    ...
    @PreAuthorize("hasAuthority('PRODUCT_MANAGER')")
    public Object approve(@PathVariable String orderId) {return lifecycle.approve(number(orderId,0));}
    ...
    @PreAuthorize("hasAuthority('PRODUCT_MANAGER')")
    public Object close(@PathVariable String orderId,jakarta.servlet.http.HttpServletRequest request) {return lifecycle.close(number(orderId,0),request.getRequestURI().matches(".*/reject/?")?"REJECT":"CANCEL",null,false);}
    ...
    @PreAuthorize("hasAuthority('PRODUCT_MANAGER')")
    public Object refund(@PathVariable String orderId) {return lifecycle.confirmVietqrRefund(number(orderId,0));}
    @PostMapping({"/api/customer/orders/{orderId}/cancel","/api/customer/orders/{orderId}/cancel/"}) @ResponseStatus(HttpStatus.CREATED)
    public Object customer(@PathVariable String orderId,@RequestParam(required=false) String token) {return lifecycle.close(number(orderId,0),"CANCEL",token,true);}
}
