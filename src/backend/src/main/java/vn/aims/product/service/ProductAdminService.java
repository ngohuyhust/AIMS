...
import vn.aims.product.repository.ProductAdminRepository;
...
public class ProductAdminService {
    ...
    public int stock(int id,int delta,String reason,String actor) {
        Product product=require(id,false);long next=(long)product.getQuantityInStock()+delta;
        ...
        repository.audit(id,"STOCK_ADJUST",Map.of("productID",id,"from",product.getQuantityInStock(),"to",next,"delta",delta),actor,reason);
    ...
    public Map<String,Object> batch(List<Integer> ids,boolean deactivate,String actor) {
        ...
        for(int id:ids) {
            ...
            String status;
            if(deactivate || product.getQuantityInStock()>0) status="DEACTIVATED";
            else status=repository.ordered(id)?"DEACTIVATED_ORDERED":"DELETED";
            if(status.equals("DELETED")) {
                repository.audit(id,"DELETE",Map.of("productID",id,"title",product.getTitle(),"productType",product.getProductType()),actor,null);
                repository.status(id,"DELETED");
            } else {
                repository.status(id,"DEACTIVATED");
                repository.audit(id,"DEACTIVATE",Map.of("productID",id,"quantityInStock",product.getQuantityInStock(),"status",status),actor,null);
            }
