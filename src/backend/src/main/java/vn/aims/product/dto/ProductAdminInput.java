...
public final class ProductAdminInput {
    ...
    public static ObjectNode parse(JsonNode body,boolean update) {
    ...
    public static String business(ObjectNode dto,Product existing) {
        boolean update=existing!=null;
        String type=update?existing.getProductType():dto.path("productType").asText();
        ...
        if(update) {
            ...
            if(dto.has("productType") && !dto.get("productType").asText().equals(existing.getProductType())) fail("productType cannot be changed");
            if(dto.has("originalPrice") && decimal(dto.get("originalPrice")).compareTo(existing.getOriginalPrice())!=0) fail("originalPrice cannot be changed");
        }
        ...
        BigDecimal original=dto.hasNonNull("originalPrice")?decimal(dto.get("originalPrice")):existing.getOriginalPrice();
        BigDecimal current=dto.hasNonNull("currentPrice")?decimal(dto.get("currentPrice")):existing.getCurrentPrice();
    ...
    public static List<Integer> ids(JsonNode body) {
    ...
    public static int delta(JsonNode body) {
