package vn.aims.product.repository;
...
public class ProductAdminRepository {
    ...
    public Product find(int id,boolean visible,boolean lock) {
    ...
    public int insert(JsonNode body) {
    ...
    public void update(int id,JsonNode body) {
    ...
    public void detail(int id,String type,JsonNode root) {
    ...
    public void audit(int id,String action,Object changes,String actor,String reason) {
    ...
    public void stock(int id,int quantity) { jdbc.update("UPDATE products SET quantity_in_stock=?,updated_at=now() WHERE product_id=?",quantity,id); }
    public void status(int id,String status) { jdbc.update("UPDATE products SET status=?,updated_at=now() WHERE product_id=?",status,id); }
    public void lockManager(String actor) { jdbc.queryForList("SELECT pg_advisory_xact_lock(hashtextextended(?, 5005))",actor); }
    public long deleteCount(String actor,Instant start,Instant end) {
    ...
    public List<Map<String,Object>> logs() {
        ...
        return logs.stream().map(log->{
            Map<String,Object> result=new LinkedHashMap<>();result.put("logID",log.getLogID());result.put("actionType",log.getActionType());
            result.put("changedFields",log.getChangedFields());result.put("performedBy",log.getPerformedBy());result.put("reason",log.getReason());
            result.put("createdAt",date.format(log.getCreatedAt()));result.put("product",log.getProduct()==null?null:ProductResponse.from(log.getProduct()));return result;
        }).toList();
    }
}
