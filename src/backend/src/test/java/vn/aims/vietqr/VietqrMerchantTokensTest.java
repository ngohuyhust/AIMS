...
class VietqrMerchantTokensTest {
    ...
    @Test void expiresAt300SecondsAndCannotActAsUserJwt() {
        ...
        assertThatThrownBy(()->later.verify("Bearer "+value)).isInstanceOf(vn.aims.payment.PaymentException.class);
        assertThatThrownBy(()->new vn.aims.auth.JwtTokens(key).verify(value)).isInstanceOf(IllegalArgumentException.class);
    }
    @Test void rejectsWrongCredentialsSignatureAndUnconfiguredMerchant() {
        ...
        assertThatThrownBy(()->tokens.issue("Basic invalid")).isInstanceOf(vn.aims.payment.PaymentException.class);
        assertThatThrownBy(()->new VietqrMerchantTokens("","",key).issue(basic)).isInstanceOf(vn.aims.payment.PaymentException.class);
        String value=(String)tokens.issue(basic).get("access_token");
        assertThatThrownBy(()->new VietqrMerchantTokens("merchant","password",key+"other").verify("Bearer "+value)).isInstanceOf(vn.aims.payment.PaymentException.class);
    }
}
