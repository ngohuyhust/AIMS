...
class PaypalInputTest {
        ...
        try(var stream=getClass().getResourceAsStream("/paypal/validation.json")) {
            for(var f:json.readTree(stream)) {
                if(f.has("errors")) {
                    ...
                    assertThatThrownBy(()->PaypalInput.parse(f.get("input"),f.path("operation").asText())).isInstanceOf(vn.aims.payment.PaymentException.class);
                } else {
    ...
    @Test void usesApprovedDecimalHalfUpAndRejectsUntrustedPaymentProof() throws Exception {
        ...
        var json=new ObjectMapper();
        for(String currency:java.util.List.of("EUR","VND","")) assertThatThrownBy(()->PaypalChecks.amount(json.readTree("{\"currency_code\":\""+currency+"\",\"value\":\"5.28\"}"),new BigDecimal("132000"))).isInstanceOf(vn.aims.payment.PaymentException.class);
        for(String id:java.util.List.of("../capture","", "a/b", "x?other")) assertThatThrownBy(()->PaypalChecks.id(json.valueToTree(id))).isInstanceOf(vn.aims.payment.PaymentException.class);
        var proof=json.readTree("{\"id\":\"P1\",\"intent\":\"CAPTURE\",\"purchase_units\":[{\"reference_id\":\"2\",\"amount\":{\"currency_code\":\"USD\",\"value\":\"5.28\"}}]}");
        assertThatThrownBy(()->PaypalChecks.order(proof,1,new BigDecimal("132000"),"P1")).isInstanceOf(vn.aims.payment.PaymentException.class);
    }
    @Test void refusesInsecureRemoteTransportAndMissingCredentials() {
        ...
        assertThatThrownBy(()->new PaypalApiClient("https://api-m.sandbox.paypal.com","","","http://localhost:4200").configured()).isInstanceOf(vn.aims.payment.PaymentException.class);
    }
}
