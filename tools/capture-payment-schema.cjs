...
entities.push(...Object.values(require(source+'/src/payment/entities/payment-transaction.entity.ts')),...Object.values(require(source+'/src/payment/entities/paypal-transaction.entity.ts')));
(async()=>{
  const db=new DataSource({type:'postgres',entities}); await db.buildMetadatas();
  const selected=db.entityMetadatas.filter(m=>m.tableName==='payment_transactions');
  ...
  const dir=root+'/src/backend/src/test/resources/payment';fs.mkdirSync(dir,{recursive:true});
