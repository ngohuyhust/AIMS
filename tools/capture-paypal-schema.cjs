...
(async()=>{
  ...
  const selected=db.entityMetadatas.filter(m=>m.tableName==='paypal_transactions');
  ...
  const dir=root+'/src/backend/src/test/resources/paypal';fs.mkdirSync(dir,{recursive:true});
